...
const Courseview = () => {
  ...
  const initialOptions = {
    clientId: "AWqF-zCg3qhcw6qwUl0q6PCTBUjyCMNujPw3UC1t3MgiiBvvJdmXXPguMnsv6AkMS5GIbIXLIuJLVycI",
    currency: "USD",
    intent: "capture",
  };
  ...
  useEffect(() => {
    const token=localStorage.getItem('userToken')
    instance
      .get(`/auth/course/${courseId}`,{headers: {
        Authorization: token
      }})
      .then((response) => {
        console.log(response);
    ...
    <PayPalScriptProvider options={initialOptions}>
      ...
      >
        ...
        >
          ...
          >
            <CardContent>
              <Typography variant="h6" gutterBottom color="textSecondary" textAlign="center">
                {course.Description}
              </Typography>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <IconButton color={isLiked ? 'secondary' : 'default'} onClick={handleLike}>
                <FavoriteIcon />
              </IconButton>
            </Box>
            {/* <Paypalpayment orderDetails={course} /> */}

            <Button onClick={()=>handlePay(course.price)} variant="contained" color="secondary" sx={{ marginTop: '20px' }}>
              Pay

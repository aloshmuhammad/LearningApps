...
const AppliedTutor = () => {
      ...
      },
      {
        ...
        options: {
          ...
          customBodyRender: (value, tableMeta, updateValue) => {
            ...
            return (
              <button style={{backgroundColor:rowData.status?'Green':'red'}} onClick={() => handleRecuruit(rowData)}>{rowData.status? 'Recruited': 'Recruit'}</button>
            );
          },
        },
      },
    ...
    useEffect(() => {
      ...
      instance
        .get('/admin/applied-tutors',{headers: {
          Authorization: token
        }})
        .then((response) => {
    ...
    const token=localStorage.getItem('Token')
      ...
      instance.post('/admin/recruit-tutor',rowData,{headers: {
        Authorization: token
      }}).then((response)=>{
          if(response?.status){

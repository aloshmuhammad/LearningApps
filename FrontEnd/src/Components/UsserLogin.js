...
const UserLogin = () => {
  ...
  const {values,errors,isSubmitting, handleChange,handleBlur,handleSubmit }=useFormik({
    ...
    onSubmit:(values)=>{
     ...
     instance.post('/auth/userlogin',body).then((response)=>{
      
      const {data}=response
      if(data?.status==false)setError('This email not Exist')
      if(data?.blocked)setError('The User is Blocked!')
      if(data?.password==false)setError('The Password you entered is incorrect')
      if(data?.google)setError('The user is already signup using google. Please login with google')
      if(data?.user){
 ...
 const handleGoogleSign=()=>{
  signInWithPopup(auth,provider).then((data)=>{
    ...
    instance.post('/auth/googleSignin',gData).then((response)=>{
